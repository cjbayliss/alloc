...
function show_all_exp($template)
{
    ...
    if ($expenseForm->get_id()) {
        if ($_POST['transactionID'] && ($_POST['edit'] || (is_object($transaction_to_edit) && $transaction_to_edit->get_id()))) {   // if edit is clicked OR if we've rejected changes made to something so are still editing it
            ...
            $query = unsafe_prepare('SELECT * FROM transaction WHERE expenseFormID=%d ORDER BY transactionID DESC', $expenseForm->get_id());
        }
        ...
        while ($db->next_record()) {
            ...
            $transaction->get_value('quantity') && ($TPL['amount'] = $transaction->get_value('amount') / $transaction->get_value('quantity'));
            ...
            $TPL['expense_class'] = $transaction->get_value('fromTfID') == config::get_config_item('expenseFormTfID') ? 'loud' : '';
...
function check_optional_allow_edit()
{
    ...
    if (is_object($expenseForm) && !$expenseForm->get_id()) {
        // New Expense Form
        $allow_edit = true;
    } elseif (is_object($expenseForm) && 1 != $expenseForm->get_value('expenseFormFinalised') && $expenseForm->get_id() && $expenseForm->is_owner()) {
        $allow_edit = true;
    } else {
        $allow_edit = false;
    }
...
function check_optional_show_line_item_add()
{
    ...
    return 1 != $expenseForm->get_value('expenseFormFinalised');
}
...
($expenseFormID = $_POST['expenseFormID']) || ($expenseFormID = $_GET['expenseFormID']);
...
if ($_POST['add']) {
    $_POST['product'] || alloc_error('You must enter a Product.');
    $_POST['companyDetails'] || alloc_error('You must enter the Company Details.');
    $_POST['fromTfID'] || alloc_error('You must enter the Source TF.');
    $_POST['quantity'] || ($_POST['quantity'] = 1);
    config::get_config_item('mainTfID') || alloc_error('You must configure the Finance Tagged Fund on the Setup -> Finance screen.');
...
if ($transaction_to_edit->get_value('fromTfID')) {
    ...
    $query = unsafe_prepare('SELECT tfID FROM tfPerson WHERE personID=%d LIMIT 1', $current_user->get_id());
    $db->query($query);

    $selectedTfID = $db->next_record() ? $db->f('tfID') : 0;
...
if ($_POST['cancel']) {
    ...
    if ('approved' == $_POST['changeTransactionStatus']) {
        $expenseForm->set_value('expenseFormComment', rtrim($expenseForm->get_value('expenseFormComment')));
        $expenseForm->save();
    ...
    if ('rejected' == $_POST['changeTransactionStatus']) {
        $expenseForm->set_value('expenseFormComment', rtrim($expenseForm->get_value('expenseFormComment')));
        $expenseForm->save();
    ...
    if ($_POST['save']) {
        $expenseForm->read_globals();
        if (0 == $expenseForm->get_value('reimbursementRequired') || 1 == $expenseForm->get_value('reimbursementRequired')) {
...
$expenseForm->get_value('paymentMethod') && ($extra = ' (' . $paymentOptionNames[$expenseForm->get_value('paymentMethod')] . ')');
...
foreach ($rr_options as $value => $label) {
    unset($extra);
    if (2 == $value) {
        $extra = ' <select name="paymentMethod">' . $paymentOptions . '</select>';
    }
...
$clientName && ($TPL['printer_clientID'] = $clientName);
