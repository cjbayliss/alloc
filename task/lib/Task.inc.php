...
class Task extends DatabaseEntity
{
    ...
    public function validate($_ = null)
    {
        ...
        if ($this->get_value('taskStatus') && !in_array($this->get_value('taskStatus'), $coerce)) {
            ...
            } elseif ('' !== $coerce[$cleaned] && '0' !== $coerce[$cleaned]) {
    ...
    public function update_children($field, $value = '')
    {
        ...
        while ($allocDatabase->row()) {
            ...
            $t->save();
            if ('Parent' == $t->get_value('taskTypeID')) {
    ...
    public function get_task_cc_list_select($projectID = ''): string
    {
        ...
        foreach ((array) $interestedPartyOptions as $email => $info) {
            if ('interested' == $info['role'] && $info['selected']) {
    ...
    public function get_name($_FORM = [])
    {
        ...
        if ('Parent' == $this->get_value('taskTypeID') && 'html' == $_FORM['return']) {
            $rtn = '<strong>' . $id . $this->get_value('taskName', DST_HTML_DISPLAY) . '</strong>';
        } elseif (isset($_FORM['return']) && 'html' == $_FORM['return']) {
    ...
    public static function get_list_filter($filter = [])
    {
        ...
        if (isset($_FORM['taskDate'])) {
            if ('new' == $filter['taskDate']) {
                ...
                // Date Created
            } elseif ('d_created' == $filter['taskDate']) {
                ...
                // Date Assigned
            } elseif ('d_assigned' == $filter['taskDate']) {
                ...
                // Date Target Start
            } elseif ('d_targetStart' == $filter['taskDate']) {
            ...
            } elseif ('d_targetCompletion' == $filter['taskDate']) {
                ...
                // Date Actual Start
            } elseif ('d_actualStart' == $filter['taskDate']) {
            ...
            } elseif ('d_actualCompletion' == $filter['taskDate']) {
    ...
    public static function get_list($_FORM): array
    {
        ...
        while ($row = $allocDatabase->next_record()) {
            ...
            if (!isset($_FORM['skipObject']) && 'array' == $_FORM['return']) {
                $row['object'] = $task;
            }
            ...
            if (isset($_FORM['taskView'])) {
                if ('byProject' == $_FORM['taskView']) {
                    $rows[$task->get_id()] = ['parentTaskID' => $row['parentTaskID'], 'row' => $row];
                } elseif ('prioritised' == $_FORM['taskView']) {
                    $rows[$row['taskID']] = $row;
    ...
    public function get_new_subtask_link()
    {
        ...
        if ('Parent' != $this->get_value('taskTypeID')) {
            return;
        }
    ...
    public function moved_from_pending_to_open()
    {
        if (is_object($this) && $this->get_id()) {
            $this->select();
            if ('open' == substr($this->get_value('taskStatus'), 0, 4)) {
                $allocDatabase = new AllocDatabase();
                $q = unsafe_prepare("SELECT *
                                FROM audit
                               WHERE taskID = %d
                ...
                return 'pending' == substr($row['value'], 0, 7);
            }
        }
    }
    ...
    public function reopen_pending_task()
    {
        if (is_object($this) && $this->get_id()) {
            $this->select();
            if ('pend' == substr($this->get_value('taskStatus'), 0, 4)) {
                $allocDatabase = new AllocDatabase();
