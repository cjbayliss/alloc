...
function show_transaction_list($transactions = [], $template)
{
    global $TPL;
    global $tflist;
    foreach ($transactions as $row) {
        ...
        if (CAN_APPROVE_TRANSACTIONS && 'admin' == $TPL['productSale_status']) {
    ...
    if (is_array($_POST['productSaleItemID']) && count($_POST['productSaleItemID'])) {
        ...
        foreach ($_POST['productSaleItemID'] as $k => $productSaleItemID) {
            ...
            if (in_array($productSaleItemID, $_POST['deleteProductSaleItem'])) {
                ...
                if ('new' == substr($productSaleItemID, 0, 3)) {
                    $productSaleItemID = '';
                }
...
if (!$productSale->get_id() || 'finished' != $productSale->get_value('status') && !('admin' == $productSale->get_value('status') && !CAN_APPROVE_TRANSACTIONS)) {
...
if ($productSaleID && 'edit' == $status) {
    define('DISPLAY', DISPLAY_PRODUCT_SALE_ITEM_EDIT);
    // Show line item + transaction + edit
} elseif ($productSaleID && ('allocate' == $status || ('admin' == $status && $productSale->have_perm(PERM_APPROVE_PRODUCT_TRANSACTIONS)))) {
    define('DISPLAY', DISPLAY_PRODUCT_SALE_ITEM_TRANSACTION_EDIT);
    // Show line item + transaction + view
} elseif ($productSaleID && ('finished' == $status || !$productSale->have_perm(PERM_APPROVE_PRODUCT_TRANSACTIONS))) {
...
} elseif ('edit' == $productSale->get_value('status')) {
...
} elseif ('allocate' == $productSale->get_value('status')) {
...
} elseif ('admin' == $productSale->get_value('status') && $productSale->have_perm(PERM_APPROVE_PRODUCT_TRANSACTIONS)) {
...
} elseif ('admin' == $productSale->get_value('status')) {
