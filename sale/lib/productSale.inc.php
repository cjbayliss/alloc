...
class productSale extends DatabaseEntity
{
    ...
    {
        $rtn = [];
        if ('admin' == $this->get_value('status') || 'finished' == $this->get_value('status')) {
            $orig = new $this->classname();
            $orig->set_id($this->get_id());
            $orig->select();
            $orig_status = $orig->get_value('status');
            if ('allocate' == $orig_status && 'admin' == $this->get_value('status')) {
    ...
    public function move_forwards()
    {
        ...
        } elseif ('admin' == $status && $this->have_perm(PERM_APPROVE_PRODUCT_TRANSACTIONS)) {
    ...
    public function move_backwards()
    {
        ...
        if ('finished' == $this->get_value('status') && $current_user->have_role('admin')) {
            $this->set_value('status', 'admin');
        } elseif ('admin' == $this->get_value('status') && $current_user->have_role('admin')) {
            $this->set_value('status', 'allocate');
        } elseif ('allocate' == $this->get_value('status')) {
    ...
    public function get_all_parties($projectID = '')
    {
        ...
        return InterestedParty::get_interested_parties('productSale', $this_id, $interestedPartyOptions);
    }
