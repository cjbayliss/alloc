...
class command
{
    ...
    public function run_commands($commands = [], $email_receive = false)
    {
        $s = null;
        $m = null;
        if ('edit_timeSheetItem' == $commands['command']) {
            [$s, $m] = $this->edit_timeSheetItem($commands);
        } elseif ('edit_task' == $commands['command']) {
            [$s, $m] = $this->edit_task($commands, $email_receive);
        } elseif ('add_comment' == $commands['command']) {
            [$s, $m] = $this->add_comment($commands);
        } elseif ('edit_reminder' == $commands['command']) {
    ...
    public function edit_task($commands, $email_receive)
    {
        ...
        if ($commands['task']) {
            ...
            $task = new Task();
            if ($commands['task'] && 'new' != strtolower($commands['task'])) {
            ...
            foreach ($commands as $k => $v) {
                ...
                if ('priority' == $k && !in_array($v, [1, 2, 3, 4, 5])) {
            ...
            if ('new' == strtolower($commands['task']) && (!$commands['desc'] && is_object($email_receive))) {
                $task->set_value('taskDescription', $email_receive->get_converted_encoding());
            }

            $after_label = 'After:  ';
            if ('new' != strtolower($commands['task'])) {
    ...
    public function edit_timeSheetItem($commands)
    {
        ...
        if ($commands['item']) {
            ...
            if ($commands['item'] && strtolower('new' != $commands['item'])) {
            ...
            $after_label2 = 'After:  ';
            if ('new' != strtolower($commands['item'])) {
    ...
    public function add_comment_via_email($commands, $email_receive)
    {
        ...
        if ($commands['key'] && $token->set_hash($commands['key'])) {
            ...
            if (is_object($entity) && 'add_comment_from_email' == $method) {
