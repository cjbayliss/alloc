...
function show_time_sheets_list_for_classes($template_name, $doAdmin = false)
{
    ...
    while ($db->next_record()) {
        ...
        if ($date && (isset($current_user->prefs["timeSheetDaysWarn"]) && (bool)strlen($current_user->prefs["timeSheetDaysWarn"])) && (mktime() - format_date("U", $date)) / 60 / 60 / 24 > $current_user->prefs["timeSheetDaysWarn"]) {
