...
class timeSheet extends DatabaseEntity
{
    ...
    public function load_pay_info()
    {
        ...
        if (null !== $this->get_value('customerBilledDollars') && (bool) strlen($this->get_value('customerBilledDollars'))) {
    ...
    public function createTransactions($status = 'pending')
    {
        ...
        if ('invoiced' != $this->get_value('status')) {
        ...
        if ($_POST['create_transactions_default'] && 0 == $this->pay_info['total_customerBilledDollars']) {
            ...
            while ($allocDatabase->next_record()) {
                ...
                } elseif (0 == $allocDatabase->f('commissionPercent')) {
    ...
    public function createTransaction($product, $amount, $tfID, $transactionType, $status = '', $fromTfID = false)
    {
        ...
        if (0 == $tfID || !$tfID || !is_numeric($tfID) || !is_numeric($amount)) {
    ...
    public static function get_list($_FORM)
    {
        ...
        while ($row = $allocDatabase->next_record()) {
            ...
            if (
                'edit' == $t->get_value('status') && (isset($current_user->prefs['timeSheetDaysWarn']) && (bool) strlen($current_user->prefs['timeSheetDaysWarn']))
    ...
    public function email_move_status_to_admin($direction, $info)
    {
        ...
        if ('forwards' == $direction) {
            ...
            if ('manager' == $this->get_value('status')) {
    ...
    public function get_all_parties($projectID = '')
    {
        ...
        return InterestedParty::get_interested_parties('timeSheet', $this_id, $interestedPartyOptions);
    }
