...
class invoice extends DatabaseEntity
{
    ...
    public function save()
    {
        ...
        if (!$this->get_value('currencyTypeID')) {
            ...
            if (!(null !== $this->get_value('maxAmount') && (bool) strlen($this->get_value('maxAmount')))) {
    ...
    public static function get_list_filter($filter = [])
    {
        ...
        // Filter invoiceID
        if (isset($filter['invoiceID'])) {
            $sql[] = sprintf_implode('invoice.invoiceID = %d', $filter['invoiceID']);
        }
        ...
        if (isset($filter['invoiceNum'])) {
            $sql[] = sprintf_implode('invoice.invoiceNum = %d', $filter['invoiceNum']);
        }

        if (isset($filter['dateOne'])) {
            $sql[] = sprintf_implode("invoice.invoiceDateFrom>='%s'", $filter['dateOne']);
        }

        if (isset($filter['dateTwo'])) {
            $sql[] = sprintf_implode("invoice.invoiceDateTo<='%s'", $filter['dateTwo']);
        }

        if (isset($filter['invoiceName'])) {
            $sql[] = sprintf_implode("invoice.invoiceName like '%%%s%%'", $filter['invoiceName']);
        }

        if (isset($filter['invoiceStatus'])) {
            $sql[] = sprintf_implode("invoice.invoiceStatus = '%s'", $filter['invoiceStatus']);
        }

        if (isset($filter['clientID'])) {
            $sql[] = sprintf_implode('invoice.clientID = %d', $filter['clientID']);
        }

        if (isset($filter['projectID'])) {
            $sql[] = sprintf_implode('invoice.projectID = %d', $filter['projectID']);
        }
    ...
    public static function get_list_filter2($filter = [])
    {
        ...
        if ('pending' == $filter['invoiceStatusPayment']) {
            $sql[] = '(COALESCE(amountPaidApproved,0) < iiAmountSum)';
        } elseif ('rejected' == $filter['invoiceStatusPayment']) {
            $sql[] = '(COALESCE(amountPaidRejected,0) > 0)';
        } elseif ('fully_paid' == $filter['invoiceStatusPayment']) {
            $sql[] = '(COALESCE(amountPaidApproved,0) = iiAmountSum)';
        } elseif ('over_paid' == $filter['invoiceStatusPayment']) {
            $sql[] = '(COALESCE(amountPaidApproved,0) > iiAmountSum)';
        }
    ...
    {
        $newstatus = $this->next_status($direction);
        if ('forwards' == $direction && 'finished' == $newstatus && $this->has_pending_transactions()) {
            alloc_error('There are still Invoice Items pending. This Invoice cannot be marked completed.');

            return false;
        }

        if ('forwards' == $direction && 'reconcile' == $newstatus) {
            $allocDatabase = new AllocDatabase();
    ...
    public function get_all_parties($projectID = '', $clientID = '')
    {
        ...
        return InterestedParty::get_interested_parties('invoice', $this->get_id(), $interestedPartyOptions);
    }
